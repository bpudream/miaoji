...
> 💡 **提示**:
> - 后端端口通过 `BACKEND_PORT` 环境变量配置，修改后需重启后端服务
> - 前端通过 `VITE_BACKEND_URL` 配置后端地址，修改后需重启前端开发服务器
> - 如果后端运行在其他机器或使用不同端口，请相应修改 `VITE_BACKEND_URL`
...
- [开发启动指南](docs/开发启动指南.md) - 开发阶段如何启动前后端服务（开发模式 vs 生产模式）

...
为了处理复杂的音视频处理流程，系统采用细粒度的状态机管理任务生命周期。

**状态流转图**:
`uploaded` -> `extracting` -> `ready_to_transcribe` -> `transcribing` -> `transcribed` -> `summarizing` -> `completed`

**状态定义**:
*   `uploaded`: 文件已上传，元数据已保存。
*   `extracting`: 正在使用 FFmpeg 提取音频。
*   `ready_to_transcribe`: 音频提取完成，等待进入转写队列。
*   `transcribing`: 正在进行 Whisper 语音转写。
*   `transcribed`: 转写完成，等待生成总结。
*   `summarizing`: 正在调用 Ollama 生成总结。
*   `completed`: 所有处理完成。
*   `error`: 发生错误（记录 `error_message` 和 `failed_stage`）。

**重试机制**:
*   根据 `failed_stage` 决定从哪个状态开始重试。
*   例如：如果在 `transcribing` 阶段失败，重试时直接从 `ready_to_transcribe` 状态开始，无需重新提取音频。

#### 2.3.4 Python Worker 架构

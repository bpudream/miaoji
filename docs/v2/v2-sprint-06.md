...
**验收标准**:
- [x] 项目详情页改为「左 60% / 右 40%」的双栏布局
- [x] 左侧再拆为上下两块（顶部播放器、底部 AI 总结）
- [x] 各面板高度固定、支持独立滚动，不出现整页滚动条

**任务拆解**:
- [x] Task-6.1.1: 重构 ProjectDetail 布局为自适应 Grid（6h）(完成时间：2024-12-XX)
- [x] Task-6.1.2: 将播放面板与总结面板拆分为可复用组件（3h）(完成时间：2024-12-XX)
- [x] Task-6.1.3: 为各面板添加固定高度与内部滚动（3h）(完成时间：2024-12-XX)
- [x] Task-6.1.4: 处理响应式断点与大屏/小屏降级策略（2h）(完成时间：2024-12-XX)
...
#### US-6.11 存储路径优化与项目名称管理
**作为** 用户
**我想要** 优化存储路径结构，并能够自定义项目显示名称
**以便** 更好地组织文件存储，并方便识别和管理项目

**验收标准**:
- [ ] 迁移文件时能够识别路径无变化的情况，前端UI给出显著提示
- [ ] 每个项目有单独的存储路径，以项目ID作为路径名
- [ ] 项目ID改为UUID格式
- [ ] 支持用户自定义项目显示名称（区别于文件名）
- [ ] 项目详情对话框中可以修改项目名称
- [ ] 上传文件前检测重复文件（MD5哈希值），提醒用户并允许查看已有项目
- [ ] 上传文件前检测重复文件，提醒用户并允许查看已有项目

**任务拆解**:

##### Task-6.11.1: 迁移文件路径无变化检测与提示
- [x] **文件**: `web/src/components/FileMigrationDialog.tsx`, `server/src/services/storage.ts` (完成时间：2024-12-XX)
- [ ] **功能**: 检测迁移目标路径与源路径是否相同
- [ ] **实现**:
  - 后端在迁移前检查源路径和目标路径是否在同一存储路径下
  - 前端显示当前文件所在路径信息
  - 如果目标路径与源路径相同，前端显示显著警告提示（黄色/橙色背景，图标提示）
  - 禁用"开始迁移"按钮或要求用户确认
- [ ] **UI提示**: 使用AlertCircle图标，黄色背景，明确文字说明"目标路径与当前路径相同，无需迁移"

##### Task-6.11.2: 存储路径结构优化 - 项目ID改为UUID
- [x] **文件**: `server/src/db.ts`, `server/src/app.ts`, `server/src/queue.ts`, `server/src/services/storage.ts`, `server/src/services/projectPath.ts` (完成时间：2024-12-XX)
- [x] **功能**: 将项目ID从INTEGER改为UUID格式 (完成时间：2024-12-XX)
- [x] **实现**: (完成时间：2024-12-XX)
  - [x] 修改`media_files`表，将`id`字段从`INTEGER PRIMARY KEY AUTOINCREMENT`改为`TEXT PRIMARY KEY` (完成时间：2024-12-XX)
  - [x] 创建数据库迁移脚本，将现有INTEGER ID转换为UUID (完成时间：2024-12-XX)
  - [x] 更新所有相关的数据库查询和API端点 (完成时间：2024-12-XX)
  - [x] 确保向后兼容性（迁移脚本自动处理旧数据） (完成时间：2024-12-XX)
- [x] **迁移策略**: (完成时间：2024-12-XX)
  - [x] 为现有项目生成UUID (完成时间：2024-12-XX)
  - [x] 更新所有外键关联（transcriptions, summaries） (完成时间：2024-12-XX)
  - [x] 自动迁移文件目录从INTEGER ID到UUID (完成时间：2024-12-XX)

##### Task-6.11.3: 存储路径结构优化 - 每个项目单独路径
- [x] **文件**: `server/src/services/storage.ts`, `server/src/app.ts`, `server/src/services/projectPath.ts` (完成时间：2024-12-XX)
- [x] **功能**: 每个项目存储在独立目录中，目录名为项目UUID (完成时间：2024-12-XX)
- [x] **实现**: (完成时间：2024-12-XX)
  - [x] 修改文件上传逻辑：`{storage_path}/{project_uuid}/original.{ext}` (完成时间：2024-12-XX)
  - [x] 修改音频提取逻辑：`{storage_path}/{project_uuid}/audio.wav` (完成时间：2024-12-XX)
  - [x] 更新`filepath`和`audio_path`存储格式 (完成时间：2024-12-XX)
  - [x] 更新文件迁移逻辑，迁移整个项目目录 (完成时间：2024-12-XX)
  - [x] 更新文件删除逻辑，删除整个项目目录 (完成时间：2024-12-XX)
  - [x] 统一路径构建逻辑到`ProjectPathService` (完成时间：2024-12-XX)
- [x] **路径格式**: (完成时间：2024-12-XX)
  - [x] 原始文件：`{base_path}/{uuid}/original.{ext}` (完成时间：2024-12-XX)
  - [x] 音频文件：`{base_path}/{uuid}/audio.wav` (完成时间：2024-12-XX)
  - [x] 未来可扩展：`{base_path}/{uuid}/transcription.json`等 (完成时间：2024-12-XX)

##### Task-6.11.4: 项目名称自定义功能
- [x] **文件**: `server/src/db.ts`, `server/src/app.ts`, `web/src/components/ProjectActionDialog.tsx` (完成时间：2024-12-XX)
- [ ] **功能**: 允许用户自定义项目显示名称
- [ ] **实现**:
  - 在`media_files`表中添加`display_name`字段（TEXT，可为NULL）
  - 创建API端点：`PUT /api/projects/:id/name`，用于更新项目名称
  - 前端在项目详情对话框中添加"编辑名称"功能
  - 项目列表和详情页优先显示`display_name`，如果为空则显示`original_name`
- [ ] **UI设计**:
  - 在项目详情对话框的基本信息区域，文件名旁边添加编辑图标
  - 点击后弹出输入框，允许修改
  - 保存后立即更新显示
  - 支持清空自定义名称，恢复为原始文件名

##### Task-6.11.5: 文件上传重复检测（MD5/哈希值）
- [ ] **文件**: `server/src/db.ts`, `server/src/app.ts`, `web/src/components/UploadDialog.tsx` 或上传相关组件
- [ ] **功能**: 上传文件前计算文件哈希值（MD5），检测是否已存在相同文件
- [ ] **实现**:
  - 在`media_files`表中添加`file_hash`字段（TEXT，可为NULL，添加索引）
  - 上传文件时，在保存到磁盘前计算文件的MD5哈希值
  - 检查数据库中是否存在相同`file_hash`的记录
  - 如果存在重复文件，返回已有项目信息（项目ID、名称、创建时间等）
  - 前端显示重复文件警告提示，提供"查看已有项目"链接
  - 允许用户选择继续上传（创建新项目）或取消上传
- [ ] **技术细节**:
  - 使用Node.js的`crypto`模块计算MD5哈希值
  - 对于大文件，使用流式读取计算哈希，避免内存溢出
  - 哈希计算在文件保存到临时目录后进行，不阻塞上传流程
  - 数据库查询使用索引优化，提高检测速度
- [ ] **UI设计**:
  - 检测到重复文件时，显示警告提示框（黄色/橙色背景，AlertCircle图标）
  - 提示内容："检测到相同文件已存在"
  - 显示已有项目信息：项目名称、创建时间、状态
  - 提供"查看已有项目"按钮（跳转到项目详情页）
  - 提供"仍要上传"按钮（继续创建新项目）
  - 提供"取消"按钮（取消上传）
- [ ] **用户体验**:
  - 哈希计算过程显示进度提示（可选）
  - 重复检测不阻塞用户操作，允许用户选择继续
  - 如果用户选择继续上传，正常创建新项目并保存哈希值

---

### Sprint 6 总结
- **总估时**: 约 95-105 小时（新增 27 小时，文件重复检测约 4-6 小时）
- **关键交付物**:
  - ✅ 多面板布局与沉浸式稿面
  - ✅ 浏览器内播放控件与段落跳转
  - ✅ 面板级滚动与响应式体验

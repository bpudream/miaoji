...
#### US-6.4 转写润色与后处理 ➡️ 已移至 Sprint 10
**作为** 用户/运营
**我想要** 在完成转写后获得可读性更强、带标点的稿件
**以便** 直接复用到文案、报告或字幕中

**状态**: 已移至 Sprint 10，详见 `v2-sprint-10.md`
...
#### US-6.5 版本管理与回溯 ➡️ 已移至 Backlog
**作为** 用户/运营
**我想要** 管理不同阶段的转写版本
**以便** 在润色和人工修改过程中随时回滚、对比、导出

**状态**: 已移至 Backlog，详见 `backlog.md`
...
**验收标准**:
- [x] 消除页面级滚动条，所有滚动限制在各面板内部
- [x] 转写内容标题栏整合所有操作按钮（编辑、润色、版本、导出），节省纵向空间
- [x] 确保各面板高度计算正确，无内容溢出

**任务拆解**:
- [x] Task-6.6.1: 修复页面级滚动条问题，确保各面板独立滚动（2h）(完成时间：2024-12-XX)
- [x] Task-6.6.2: 将编辑/润色/版本/导出按钮整合到转写标题栏（2h）(完成时间：2024-12-XX)
- [x] Task-6.6.3: 优化面板高度计算与响应式布局（1h）(完成时间：2024-12-XX)
...
**验收标准**:
- [x] 编辑、润色、版本、导出按钮统一放在转写内容标题栏
- [x] 复制按钮保留在标题栏
- [x] 移除独立的操作按钮行，节省纵向空间

**任务拆解**:
- [x] Task-6.8.1: 重构 TranscriptionResult 组件，将操作按钮移至标题栏（2h）(完成时间：2024-12-XX)
- [x] Task-6.8.2: 优化按钮布局与视觉层次（1h）(完成时间：2024-12-XX)
...
**验收标准**:
- [x] 在系统设置页面中新增"系统状态"区域
- [x] 显示当前服务器 IP 地址（前端和后端）
- [x] 显示完整的访问链接（前端链接），支持一键复制
- [x] 支持生成二维码，方便手机扫码访问
- [x] 显示后端 API 地址（供开发者参考）

**任务拆解**:
- [x] Task-6.9.1: 后端实现获取服务器 IP 地址和端口信息的 API（2h）(完成时间：2024-12-XX)
- [x] Task-6.9.2: 前端实现系统状态页面 UI，显示 IP 和链接（3h）(完成时间：2024-12-XX)
- [x] Task-6.9.3: 集成二维码生成库，实现扫码功能（2h）(完成时间：2024-12-XX)
- [x] Task-6.9.4: 实现链接复制功能（1h）(完成时间：2024-12-XX)
...
- [x] **取消支持**: 支持取消正在进行的批量迁移（已放弃，不在计划中）
...
**验收标准**:
- [x] 迁移文件时能够识别路径无变化的情况，前端UI给出显著提示
- [x] 每个项目有单独的存储路径，以项目ID作为路径名
- [x] 项目ID改为UUID格式
- [x] 支持用户自定义项目显示名称（区别于文件名）
- [x] 项目详情对话框中可以修改项目名称
- [x] 上传文件前检测重复文件（MD5哈希值），提醒用户并允许查看已有项目

**任务拆解**:
- [x] Task-6.11.1: 迁移文件路径无变化检测与提示（2h）(完成时间：2024-12-XX)
- [x] Task-6.11.2: 存储路径结构优化 - 项目ID改为UUID（4h）(完成时间：2024-12-XX)
- [x] Task-6.11.3: 存储路径结构优化 - 每个项目单独路径（4h）(完成时间：2024-12-XX)
- [x] Task-6.11.4: 项目名称自定义功能（3h）(完成时间：2024-12-XX)
- [x] Task-6.11.5: 文件上传重复检测（MD5/哈希值）（6h）(完成时间：2024-12-XX)
...
- **关键交付物**:
  ...
  - ✅ 文件上传重复检测（MD5哈希值）
- **已移至其他 Sprint**:
  - ➡️ US-6.4 转写润色与后处理 → Sprint 10
  - ➡️ US-6.5 版本管理与回溯 → Backlog
- **已放弃**:
  - ❌ US-6.3 AI 面板增强（可选）

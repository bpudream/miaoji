...
### US-8.5 后端日志系统
- [ ] **Task 8.5.1**: 选择并集成日志库（推荐 Winston 或 Pino），配置日志级别和输出格式（2h）
- [ ] **Task 8.5.2**: 配置日志输出到文件（`logs/app.log`）和控制台，支持不同环境的不同配置（2h）
- [ ] **Task 8.5.3**: 实现日志轮转功能，按日期或大小自动轮转日志文件，避免单文件过大（2h）
- [ ] **Task 8.5.4**: 在关键位置添加日志记录（API 请求/响应、错误处理、队列任务、文件操作等）（3h）
- [ ] **Task 8.5.5**: 添加请求ID追踪，确保同一请求的日志可以关联（1h）
- [ ] **Task 8.5.6**: 配置生产环境日志级别（仅记录 error、warn、info），开发环境可记录 debug（1h）
...
7.  **日志文件**：日志文件存储在 `logs/` 目录，支持按日期轮转。
8.  **日志级别**：生产环境仅记录 error、warn、info，开发环境可记录 debug。
9.  **问题排查**：通过日志可以追踪请求流程、定位错误原因。
